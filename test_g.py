...
e = [e for e in all_emails if e[1] in ['petr.borisovbvl@mail.ru', 'lev.uvarovedk@mail.ru', 'prokhor.savelevgvc@mail.ru', 'viacheslav.popovmom@mail.ru', 'ruslan.ezhovqzw@mail.ru', 'gidorkandxasro4537@mail.ru']]
for em in e:
    times = ''
    links = ''
    surnames = ''
    ...
    # soup = gmm.find_regex_in_email_with_title(username, password, 'Подтверждение записи', "SEEN")
    for s in soup:
        # telegram.send_doc('VOLKAVA_NADZEYA.html', str(s))
        # print(str(s).replace('&amp;', '&').replace('request_locale=de', 'request_locale=ru'))
